...
export async function fetchPreviewImage(target: string): Promise<{ body: Buffer; type: string }> {
  ...
  if (body.length > MAX_IMAGE) throw new Error('Preview image is too large')
  return { body, type: type.startsWith('image/') ? type : 'image/jpeg' }
}

export async function fetchRemoteMedia(target: string): Promise<{ body: Buffer; type: string }> {
  const url = publicUrl(target)
  const response = await fetch(url, {
    redirect: 'follow',
  ...
  const body = Buffer.from(await response.arrayBuffer())
  if (body.length > MAX_REMOTE_MEDIA) throw new Error('Media file is too large')
  const resolved =
...
function readSearch(req: IncomingMessage): URLSearchParams {
  return new URL(req.url ?? '/', 'http://127.0.0.1').searchParams
}

async function handlePreview(req: IncomingMessage, res: ServerResponse) {
  const target = readSearch(req).get('url')
  if (!target) {
    sendJson(res, 400, { error: 'Missing url' })
    return
  }
  try {
    sendJson(res, 200, await previewLink(target))
  } catch (error) {
    sendJson(res, 502, { error: error instanceof Error ? error.message : 'Preview failed' })
  }
}
...
async function handleMedia(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    sendJson(res, 502, { error: error instanceof Error ? error.message : 'Media failed' })
  }
}
...
async function handleImage(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    sendJson(res, 502, { error: error instanceof Error ? error.message : 'Image failed' })
  }
}
...
export function linkPreviewPlugin(): Plugin {
  const attach = (server: { middlewares: { use: (fn: (req: IncomingMessage, res: ServerResponse, next: () => void) => void) => void } }) => {
    server.middlewares.use((req, res, next) => {
      ...
      if (path === '/api/link-image') {
        void handleImage(req, res)
        return
      }
      if (path === '/api/link-media') {
        void handleMedia(req, res)
        return
      }
      next()
    })
